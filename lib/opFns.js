...
module.exports = {
  ...
  EXP: function (base, exponent, runState) {
    ...
    if (!exponent.isZero()) {
      let bytes = 0
      for(let v = exponent; !v.isZero(); bytes++) {
        v = v.shrn(8)
      }
  ...
  SHA3: function (offset, length, runState) {
    ...
    const data = memLoad(runState, offset, length)
    // copy fee
  ...
  CALLDATALOAD: function (pos, runState) {
    pos = utils.bufferToInt(pos)
    let loaded = runState.callData.slice(pos, pos + 32)
  ...
  CALLDATACOPY: function (memOffset, dataOffsetBuf, dataLength, runState) {
    ...
    const dataOffset = utils.bufferToInt(dataOffsetBuf)
  ...
  EXTCODESIZE: function (address, runState, cb) {
    const stateManager = runState.stateManager
    address = utils.setLengthLeft(address, 20)
    stateManager.getContractCode(address, (err, code) => {
  ...
  BLOCKHASH: function (number, runState, cb) {
    ...
    const diff = utils.bufferToInt(runState.block.header.number) - utils.bufferToInt(number)
    ...
    stateManager.getBlockHash(number, (err, blockHash) => {
      if (err) {
  ...
  MLOAD: function (pos, runState) {
    pos = utils.bufferToInt(pos)
    const loaded = utils.unpad(memLoad(runState, pos, 32))
    return loaded
  },
  ...
  SLOAD: function (key, runState, cb) {
    ...
    stateManager.getContractStorage(runState.address, key, (err, value) => {
  ...
  SSTORE: function (key, val, runState, cb) {
    ...
    key = utils.setLengthLeft(key, 32)
    const value = utils.unpad(val)

    stateManager.getContractStorage(runState.address, key, (err, found) => {
      if (err) return cb(err)
      try {
      ...
      stateManager.putContractStorage(address, key, value, (err) => {
        if (err) return cb(err)
  ...
  JUMPI: function (c, i, runState) {
    ...
    const dest = i ? c : runState.programCounter
  ...
  SWAP: function (runState) {
    ...
    // preform the swap
    const newTop = runState.stack[swapIndex]
  ...
  CALL: function (gasLimit, toAddress, value, inOffset, inLength, outOffset, outLength, runState, done) {
    ...
    const data = memLoad(runState, inOffset, inLength)
    const options = {
      gasLimit: gasLimit,
      value: value,
      to: toAddress,
    ...
    stateManager.exists(toAddress, (err, exists) => {
      if (err) {
        done(err)
        return
      }
  ...
  CALLCODE: function (gas, toAddress, value, inOffset, inLength, outOffset, outLength, runState, done) {
    ...
    if (utils.isPrecompiled(toAddress)) {
      ...
      stateManager.getContractCode(toAddress, (err, code, compiled) => {
  ...
  DELEGATECALL: function (gas, toAddress, inOffset, inLength, outOffset, outLength, runState, done) {
    ...
    // load the code
    stateManager.getAccount(toAddress, (err, account) => {
      if (err) return done(err)
      if (utils.isPrecompiled(toAddress)) {
        options.compiled = true
        options.code = runState._precompiled[toAddress.toString('hex')]
  ...
  SUICIDE: function (suicideToAddress, runState, cb) {
    ...
    const contractAddress = runState.address
    suicideToAddress = utils.setLengthLeft(suicideToAddress, 20)

    stateManager.getAccount(suicideToAddress, (err, toAccount) => {
      // update balances
      if (err) {
        cb(err)
        return
      ...
      const newBalance = new Buffer(new BN(contract.balance).add(new BN(toAccount.balance)).toArray())
      async.series([
...
function describeLocation (runState) {
  const hash = utils.sha3(runState.code).toString('hex')
  ...
  function parseCallResults (err, results) {
    ...
    if (!results.vm.exceptionError) {
      ...
      if (results.vm.return) {
        for (let i = 0; i < Math.min(localOpts.outLength, results.vm.return.length); i++) {
          runState.memory[localOpts.outOffset + i] = results.vm.return[i]
        }
      }

      // update stateRoot on current contract
      runState.stateManager.getAccount(runState.address, (err, account) => {
