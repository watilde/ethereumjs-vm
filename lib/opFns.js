...
module.exports = {
  ...
  MLOAD: function (pos, runState) {
    pos = new BN(pos)
    return utils.unpad(memLoad(runState, pos, new BN(32)))
  },
  ...
  MSIZE: function (runState) {
    return runState.memoryWordCount.muln(32).toArrayLike(Buffer, 'be', 32)
  },
...
function subMemUsage (runState, offset, length) {
  ...
  // fill the expanded memory
//  console.log('filling resize', runState.memoryWordCount, newMemoryWordCount)
//  for (var i = runState.memoryWordCount.toNumber() * 32; i < newMemoryWordCount.toNumber() * 32; i++) {
//    runState.memory[i] = 0
//  }
...
function memStore (runState, offset, val, valOffset, length, skipSubMem) {
  ...
  if (length > safeLen) {
    for (var i = 0; i < length - safeLen; i++) {
//      runState.memory[offset + safeLen + i] = 0
    }
  }
}
