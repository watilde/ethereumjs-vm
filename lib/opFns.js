...
function memStore (runState, offset, val, valOffset, length, skipSubMem) {
  ...
  if (valOffset.ltn(val.length)) {
    // NOTE: toNumber() is safe here as it will be at most as big as val.length
    valOffset = valOffset.toNumber()

    // read max possible from the value
    for (; i < Math.min(val.length - valOffset, length); i++) {
      runState.memory[offset + i] = val[valOffset + i]
    }
  }

  if (i != length)
    console.log('memStore topping up', length - i, i < length)
