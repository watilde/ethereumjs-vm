...
module.exports = function (opts, cb) {
  const self = this
  const stateManager = self.stateManager
  ...
  function loadContract (cb) {
    stateManager.getAccount(runState.address, (err, account) => {
  ...
  function iterateVm (done) {
    ...
    const opInfo = lookupOpInfo(opCode)
    ...
    function runOp (cb) {
      ...
      const argsNum = opInfo.in
      // pop the stack
      const args = argsNum ? runState.stack.splice(-opInfo.in) : []
      args.reverse()
      args.push(runState)
